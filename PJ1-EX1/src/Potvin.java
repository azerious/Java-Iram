...
public class Potvin {
	...
	public static void main(String[] args) {
		...
		//'utilise des for dans tout mon code car je parcour des tableaux dont je connais la longueur à chaque fois
	...
	public static String constructionAffichage(String affichageFinal, String WEEKS, int maxlengthToRanks,
		...
		finalAffichage = placeNumberToRank(finalAffichage, WEEKS, 21);
		finalAffichage = constructTendance(finalAffichage, pourcentages, maxlengthToRanks, datasCurrent, datasPrevious);
		finalAffichage = placeNumberToRank(finalAffichage, status, 54);
	...
	/**
	 *  Permet de verifier le nivreau de status des pourcentages de cas (rouge, vert, orange)
	 * @param datasCurrent
	 * @param datasPrevious
	 * @return
	 */
	public static String checkStatus(int[] datasCurrent, int[] datasPrevious) {
		...
		for (int i = 0; i < datasCurrent.length; i++) {
			// Si le montant de cas / admissions sont suppérieur à la semaine précédante alors incrementer le compteur de statuschekcer
	...
	public static void header(String[] HEADER, int longueurRank) {
		String finalHeader = HEADER[0];

		for (int i = 1; i < HEADER.length; i++) {
			finalHeader = placeNumberToRank(finalHeader, HEADER[i], 10+longueurRank*i);
		}
	...
	public static String constructTendance(String str, double[] pourcents, int longueurMax, int[] datasCurrent,
		...
		for (int i = 0; i < datasCurrent.length; i++) {
			// Utilisation d'une ternairep our adapter le signe devant le pourcentage en fonction des données
